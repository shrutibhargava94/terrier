...
public class TerrierWrapper {
	...
	public void setStopwordRemoval(String lang) {
		...
		System.setProperty("stopwords.filename", "");
	...
	public ResultSet getResultSet(String query, 
		...
		System.setProperty("tokeniser","NGramTokeniser");
	...
	*/
	...
	public void prepareIndex(String index_path, String prefix, 
		...
		if(true || collection_extention.endsWith("txt") || collection_extention.endsWith("text")||collection_extention.endsWith("utf8"))
			System.setProperty("trec.collection.class", "NYTCollection");
		if(!stopword_removal && !stem)
			System.setProperty("termpipelines", "");
		if(stopword_removal) {
			this.setStopwordRemoval(lang);
	...
	public void prepareIndex(String path_to_data, 
		...
		this.index = Index.createIndex(this.path_to_index, this.indexPrefix);
		updateCollectionStatistics();
			...
			if(f.isDirectory()) {
				...
				for(File f1: F1Dir )  {
					if(f1.isDirectory()) {
						...
						for(File f2: F2Dir) {
							if(f2.isDirectory()){
								File[] F3Dir = f2.listFiles();
								for(File f3:F3Dir){
									p.println(f3.getAbsolutePath());
								}
							}else
